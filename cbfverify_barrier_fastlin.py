...
def get_upper_coefficients(UB,LB,activation="relu"):
  ...
  # A_U, B_U
  # A_U: (num_hidden, num_hidden)
  # B_U: (num_hidden, 1)
  # A_U_diag: (num_hidden, 1)
  A_U_diag = torch.zeros(UB.shape[0],1)
...
def get_lower_coefficients(UB,LB,activation="relu",method="fastlin"):
  ...
  # B_U: (num_hidden, 1)
  # A_U_diag: (num_hidden, 1)
  A_L_diag = torch.zeros(UB.shape[0],1)
  ...
  # A_L: (num_hidden, num_hidden)
  A_L = torch.diag(A_L_diag[:,0])

  return A_L, B_L

def get_barrier_bounds_fastlin(x0, eps, Ws, bs, method="fastlin", activation="relu"):
  # x0: (dim_in, num_batch)
  # initial setting
  # act = lambda x: torch.relu(x) 
  num_layer = len(Ws)
  ...
  for i in range(num_layer):
    if i == 0:
      ...
      W_pos[Ws[i]<0]=0
      W_neg[Ws[i]>0]=0
      UB=torch.matmul(W_pos,UBs[i])+torch.matmul(W_neg,LBs[i])+bs[i] # i=0, (num_hidden, num_batch=1)
      ...
      A_U, B_U = get_upper_coefficients(UBs[-1],LBs[-1],activation) # A_U: (num_hidden, num_hidden)
      A_L, B_L = get_lower_coefficients(UBs[-1],LBs[-1],activation,method) # B_U: (num_hidden, 1)

      A_U_new = torch.matmul(W_pos,A_U) + torch.matmul(W_neg,A_L) # shape: (dim_out, num_hidden)
      B_U_new = torch.matmul(W_pos,B_U) + torch.matmul(W_neg,B_L) + bs[i] # shape: (dim_out, 1)
      A_L_new = torch.matmul(W_pos,A_L) + torch.matmul(W_neg,A_U)
      B_L_new = torch.matmul(W_pos,B_L) + torch.matmul(W_neg,B_U) + bs[i]

      # for 2-layer UB:
      W_equiv_U = torch.matmul(A_U_new, Ws[i-1]) # shape: (dim_out, dim_in)
      B_equiv_U = torch.matmul(A_U_new, bs[i-1]) + B_U_new # shape: (dim_out, 1)
      W_pos = W_equiv_U.clone()
      W_neg = W_equiv_U.clone()
      W_pos[W_equiv_U<0]=0
      W_neg[W_equiv_U>0]=0
      UB=torch.matmul(W_pos,UBs[0])+torch.matmul(W_neg,LBs[0])+B_equiv_U # shape: (dim_out, batch=1)
      ...
      W_pos[W_equiv_L<0]=0
      W_neg[W_equiv_L>0]=0
      LB=torch.matmul(W_pos,LBs[0])+torch.matmul(W_neg,UBs[0])+B_equiv_L # shape: (dim_out, batch=1)
